...
def is_seg_file(p: Path) -> bool:
    return "_seg.nii" in p.name.lower()
...
def collect_hgg_subjects(train_root: Path) -> list[Path]:
    """
    Return subject dirs under Brats17TrainingData/HGG only.
    """
    hgg_root = train_root / HGG_DIR_NAME
    if not hgg_root.exists():
        raise FileNotFoundError(f"Missing HGG dir: {hgg_root}")
    subjects = [p for p in hgg_root.iterdir() if p.is_dir()]
...
def make_folds(subject_dirs: list[Path], seed: int = 2025) -> dict[str, list[Path]]:
    ...
    n = len(subject_dirs)
    assert n == 210, f"Expected 210 HGG subjects, but got {n}"

    # split into 5 equal folds (210 → 42 each)
...
def main():
    ...
    subjects = collect_hgg_subjects(in_train_root)
    if not subjects:
        raise RuntimeError(f"No HGG subjects found in {in_train_root/HGG_DIR_NAME}")

    if len(subjects) != 210:
        print(f"[Warn] Found {len(subjects)} HGG subjects (expected 210). Proceeding anyway.")

    folds = make_folds(subjects, seed=SEED)

    # Save manifest with ONLY HGG subjects
    ...
    for fold_name, subj_dirs in tqdm(folds.items()):
        ...
        existing = [p for p in fold_out_root.glob("Brats17_*") if p.is_dir()]
    ...
    print(
        "Done.\n"
        f"  {count_mod} modality volumes → PNG views (HGG only)\n"
